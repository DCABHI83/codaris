...
const Pricing = () => {
  ...
  const oneTimePlans = [
    ...
    },
    {
      ...
      features: [
        ...
        'Unlimited revisions',
        '30-day delivery',
        'Free hosting on Render/Railway',
  ...
  const plans = isRental ? rentalPlans : oneTimePlans;
    ...
    <>
      <div className='py-16 px-4'>
        ...
        <div className='flex items-center justify-center gap-4 mb-16'>
          <span className={`text-lg font-medium transition-colors ${!isRental ? 'text-black' : 'text-gray-400'}`}>
            One-Time
          </span>
          
          <button
            onClick={() => setIsRental(!isRental)}
            className='relative w-20 h-10 bg-gray-200 rounded-full p-1 transition-colors duration-300 hover:bg-gray-300'
            style={{ backgroundColor: isRental ? '#000' : '#e5e7eb' }}
          >
            <motion.div
          ...
          {plans.map((plan, index) => (
            ...
            >
              ...
              <button
                ...
                `}
              >
                {isRental ? 'Start Free Trial' : 'Get Started'}
              </button>
            </motion.div>
          ))}
        </div>

        {/* Rental Benefits - Only show when rental is active */}
        {isRental && (
        ...
        <div className='max-w-4xl mx-auto mt-12 p-6 bg-blue-50 rounded-xl border border-blue-200'>
          <div className='flex items-start gap-3'>
            ...
            <div>
              <h4 className='font-semibold mb-1'>🚀 Free Hosting Included</h4>
              <p className='text-sm text-gray-700'>
                All plans include free hosting on platforms like Vercel, Netlify, or Render. Perfect for small to medium traffic. 
                <span className='font-medium'> For high-traffic websites, we'll help you migrate to a VPS server</span> (hosting costs will apply separately).
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Payment Note */}
        <div className='max-w-3xl mx-auto mt-8 p-6 bg-gray-50 rounded-xl border border-gray-200'>
          <p className='text-sm text-gray-600 text-center'>
            {isRental 
              ? '🎉 Special Launch Offer! Get your first month at an introductory price. All plans include hosting, maintenance, and updates!'
              : '💳 Payment Terms: 50% advance, 50% on delivery • All prices include GST • Free revisions within 30 days of delivery'
            }
          </p>
        </div>
      </div>
